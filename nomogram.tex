...
Traces of the Brier score over time indicated that all models produced survival estimates superior to KM0 from approximately 100 days to 600 days post-diagnosis (\fref{fig:nomo-brier-paths}).  The performance of the models was similar, with RSF displaying slightly higher error rates at longer follow-up times, and models CP1 and GG1 demonstrating comparable performance.  Bootstrapped differences between KM0 and model \Gls{IBS} further highlighted that although all models were superior to KM0, RSF had noticeably lower average performance than CP1 and GG1, which were in turn nearly equivalent in terms of performance (\fref{fig:nomo-ibs-delta}).

\begin{figure}
\centering
  \includegraphics[width=.7\linewidth]{analysis/nomogram/figure/05-model-selection-ibs-3}
  \caption[Bootstrapped differences in \acrshort{IBS} between candidate models and KM0]{Distribution of bootstrapped differences in \gls{IBS} between candidate models and null-model KM0.  For each of 500 bootstrap draws from the \gls{NSWPCN} holdout model test data, the difference between the \gls{IBS} of a candidate model, and the \gls{IBS} of KM0, was calculated as $\Delta {IBS}_Model = IBS_KM0 - IBS_Model$.  The smoothed distributions of $\Delta {IBS}_Model$ are shown here for each candidate model.  All models predicted outcome better than KM0 in most draws, and models CP1 (green) and GG (blue) both had very similar performance, which was on average, but not significantly, better than that of RSF (purple). }
