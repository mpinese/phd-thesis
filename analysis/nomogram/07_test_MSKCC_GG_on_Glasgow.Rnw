...
gg.linpreds = -rowSums(gg.linpreds)		# Negate to bring into concordance with the direction of Cox coefficients (ie higher is now worse)

gg.linpreds.train = sapply(1:length(gg.coefs), function(coef_i) {
	if (names(gg.coefs)[coef_i] %in% colnames(fit.final$gg$data$m)) { 
		gg.coefs[coef_i] * fit.final$gg$data$m[,names(gg.coefs)[coef_i]] 
	} else if (gsub("TRUE$", "", names(gg.coefs)[coef_i]) %in% colnames(fit.final$gg$data$m)) { 
		gg.coefs[coef_i] * fit.final$gg$data$m[,gsub("TRUE$", "", names(gg.coefs)[coef_i])] 
	} else {
		rep(0, nrow(fit.final$gg$data$m)) 
	} })
gg.linpreds.train = -rowSums(gg.linpreds.train)		# Negate to bring into concordance with the direction of Cox coefficients (ie higher is now worse)
@
...
<<altman-score-hists>>=
par(mfrow = c(2, 1))
hist(gg.linpreds.train, main = "NSWPCN GG scores", xlim = range(c(gg.linpreds.train, gg.linpreds)), breaks = 20, col = "grey")
abline(v = quantile(gg.linpreds.train, probs = c(0.25, 0.5, 0.75)), col = "red")
hist(gg.linpreds, main = "Glasgow GG scores", xlim = range(c(gg.linpreds.train, gg.linpreds)), breaks = 20, col = "grey")
abline(v = quantile(gg.linpreds, probs = c(0.25, 0.5, 0.75)), col = "red")
par(mfrow = c(1, 1))
@
...
anova(coxph(Surv(History.Death.EventTimeDays, History.DSDeath.Event) ~ offset(gg.linpreds) + gg.linpreds, data.glasgow))
@
...
<<altman-2>>=
summary(coxph(Surv(History.Death.EventTimeDays, History.DSDeath.Event) ~ offset(mskcc.scores.glasgow.postop[,1]) + AgeCent + SexM + SizeCent + A2 + A4, data.glasgow))
#summary(coxph(Surv(History.Death.EventTimeDays, History.DSDeath.Event) ~ offset(mskcc.scores.glasgow.preop[,1]) + AgeCent + SexM + SizeCent + A2 + A4, data.glasgow))
summary(coxph(Surv(History.Death.EventTimeDays, History.DSDeath.Event) ~ offset(gg.linpreds) + AgeCent + SexM + SizeCent + A2 + A4, data.glasgow))
@

Strong evidence of misspecification or poor fit.  However, the above calibration slope was not significantly different from 1.  Hmm.

This doesn't necessarily sink the method, but will need checking as we go along.
...
<<altman-4>>=
# Split into four risk groups by quantiles: 20%, 50%, 80%
groups.quantiles = c(0, 0.2, 0.5, 0.8, 1)
groups.train = cut(gg.linpreds.train, quantile(gg.linpreds.train, groups.quantiles))
groups.test.gg = cut(gg.linpreds, quantile(gg.linpreds, groups.quantiles))
groups.test.mskcc_preop = cut(mskcc.scores.glasgow.preop[,1], quantile(mskcc.scores.glasgow.preop[,1], groups.quantiles))
groups.test.mskcc_postop = cut(mskcc.scores.glasgow.postop[,1], quantile(mskcc.scores.glasgow.postop[,1], groups.quantiles))
par(mfrow = c(2,2))
plot(survfit(fit.final$gg$data$m[,1] ~ groups.train), col = 1:4)
plot(survfit(Surv(History.Death.EventTimeDays, History.DSDeath.Event) ~ groups.test.gg, data = data.glasgow), col = 1:4)
plot(survfit(Surv(History.Death.EventTimeDays, History.DSDeath.Event) ~ groups.test.mskcc_preop, data = data.glasgow), col = 1:4)
plot(survfit(Surv(History.Death.EventTimeDays, History.DSDeath.Event) ~ groups.test.mskcc_postop, data = data.glasgow), col = 1:4)
par(mfrow = c(1,1))
@
