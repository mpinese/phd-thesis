...
names(pal) = c("GG", "GG2", "CPH", "RSF", "KM0")
@
...
<<data-selection>>=
x = data[,c("History.Diagnosis.Date", "Patient.Sex", "History.Diagnosis.AgeAt.Cent", "Path.LocationBody", "Path.Size.Cent", "Path.Ca199.Preop", "Molec.S100A2.DCThresh", "Molec.S100A4.DCThresh")]
colnames(x) = c("DiagYearCent", "SexM", "AgeCent", "LocBody", "SizeCent", "Ca199", "A2", "A4")
x$SexM = x$Sex == "M"
x$Ca199 = x$Ca199 > 100
median(x$DiagYearCent)
x$DiagYearCent = as.numeric((x$DiagYearCent - median(x$DiagYearCent)) / 365.25)
hist(x$DiagYearCent, main = "Histogram of Median-Centered Diagnosis Year", xlab = "")
...
temp = NA
temp = ls()
rm(list = temp[!(temp %in% c("x", "y", "pal"))])
...
# Remove long-survivors.  These are very likely to be misdiagnoses, or LTF.
nrow(data)
...
set.seed(20150208)
fit.cph.as.bic = glmulti(Surv(Time, DSD) ~ strata(SexM) + AgeCent + LocBody + SizeCent + SizePlus + A2 + A4, data = data, marginality = TRUE, method = "g", fitfunction = "coxph", crit = "bic", level = 2, plotty = FALSE, report = TRUE)
...
set.seed(20150208)
fit.cph.as.aicc = glmulti(Surv(Time, DSD) ~ strata(SexM) + AgeCent + LocBody + SizeCent + SizePlus + A2 + A4, data = data, marginality = TRUE, method = "g", fitfunction = "coxph", crit = "aicc", level = 2, plotty = FALSE, report = TRUE)
...
rm(nobs.coxph)
summary(fit.cph.as.bic)$bestmodel
summary(fit.cph.as.aicc)$bestmodel
...
temp.data = rbind(temp.data, data.frame(time = rep(temp.preds.rsf$time.interest, each = nrow(temp.preds.rsf$survival)), surv = as.vector(temp.preds.rsf$survival), upper = NA, lower = NA, group = rep(temp.grid$ID, length(temp.preds.rsf$time.interest)), model = "RSF"))
...
plotAUCcurve(temp.gg2, conf.int = FALSE, add = TRUE, col = pal["GG2"])
plotAUCcurve(temp.rsf, conf.int = FALSE, add = TRUE, col = pal["RSF"])
plotAUCcurve(temp.cph, conf.int = FALSE, add = TRUE, col = pal["CPH"])
legend("topright", legend = c("GG", "GG2", "RSF", "CPH"), col = pal[c("GG", "GG2", "RSF", "CPH")], lty = "solid")
abline(v = c(7, 34))
...
invisible(stdca(data = temp.data, outcome = "DSD", ttoutcome = "Time", predictors = c("gg.1", "gg2.1", "cph.1", "rsf.1"), timepoint = 365, probability = rep(TRUE, 4)))
invisible(stdca(data = temp.data, outcome = "DSD", ttoutcome = "Time", predictors = c("gg.2", "gg2.2", "cph.2", "rsf.2"), timepoint = 365*2, probability = rep(TRUE, 4)))
invisible(stdca(data = temp.data, outcome = "DSD", ttoutcome = "Time", predictors = c("gg.3", "gg2.3", "cph.3", "rsf.3"), timepoint = 365*3, probability = rep(TRUE, 4)))
@
...
ggplot(temp, aes(x = Time/365.25*12, y = BS, colour = Model)) + geom_line() + ylab("Brier Score") + geom_hline(yintercept = 0.25) + geom_vline(xintercept = c(7, 34))
...
ggplot(temp, aes(x = Time/365.25*12, y = Q50, ymin = Q5, ymax = Q95, colour = Model, fill = Model)) + geom_line() + geom_ribbon(alpha = 0.2, colour = NA) + ylab("Brier Score, 90\\% BI") + geom_hline(yintercept = 0.25) + geom_vline(xintercept = c(7, 34))
...
temp$Quantile = paste("Q", gsub("%", "", temp$Quantile), sep = "")
temp = dcast(temp, Model + Time ~ Quantile, value.var = "Value")
ggplot(temp, aes(x = Time/365.25*12, y = Q50, ymin = Q5, ymax = Q95, colour = Model, fill = Model)) + geom_line() + geom_ribbon(alpha = 0.2, colour = NA) + ylab("Brier Score: Improvement over KM0. BS median, 90\\% BI") + geom_hline(yintercept = 0)
ggplot(temp, aes(x = Time/365.25*12, y = Q50, ymin = Q5, ymax = Q95, colour = Model, fill = Model)) + geom_line() + geom_ribbon(alpha = 0.2, colour = NA) + xlim(0, 34) + ylab("Brier Score: Improvement over KM0. BS median, 90\\% BI") + geom_hline(yintercept = 0)
ggplot(temp, aes(x = Time/365.25*12, y = Q50, colour = Model)) + geom_line() + ylab("Brier Score: Improvement over KM0, BS median") + geom_hline(yintercept = 0) + geom_vline(xintercept = c(7, 34))
ggplot(temp, aes(x = Time/365.25*12, y = Q50, colour = Model)) + geom_line() + ylab("Brier Score: Improvement over KM0, BS median") + xlim(0, 34) + geom_hline(yintercept = 0) + geom_vline(xintercept = c(7, 34))
@
...
ibsc_boots = t(sapply(1:5e2, function(i) {
	...
	boot_samp = sample.int(nrow(data.val), replace = TRUE)
	gg = calcIBS(Surv(data.val$Time, data.val$DSD)[boot_samp,], ibs_preds_gg[boot_samp,], ibs_times, 34*365.25/12, 7*365.25/12)$ibs
	gg2 = calcIBS(Surv(data.val$Time, data.val$DSD)[boot_samp,], ibs_preds_gg2[boot_samp,], ibs_times, 34*365.25/12, 7*365.25/12)$ibs
	cph = calcIBS(Surv(data.val$Time, data.val$DSD)[boot_samp,], ibs_preds_cph[boot_samp,], ibs_times, 34*365.25/12, 7*365.25/12)$ibs
	rsf = calcIBS(Surv(data.val$Time, data.val$DSD)[boot_samp,], ibs_preds_rsf[boot_samp,], ibs_times, 34*365.25/12, 7*365.25/12)$ibs
	km0 = calcIBS(Surv(data.val$Time, data.val$DSD)[boot_samp,], ibs_preds_km0[boot_samp,], ibs_times, 34*365.25/12, 7*365.25/12)$ibs
...
<<model-selection-ibs>>=
calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_gg, ibs_times, 34*365.25/12, 7*365.25/12)$ibs
calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_gg2, ibs_times, 34*365.25/12, 7*365.25/12)$ibs
calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_cph, ibs_times, 34*365.25/12, 7*365.25/12)$ibs
calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_rsf, ibs_times, 34*365.25/12, 7*365.25/12)$ibs
calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_km0, ibs_times, 34*365.25/12, 7*365.25/12)$ibs

boxplot(ibsc_boots, main = "IBS BS Distribution", ylab = "IBS")
plot(density(ibsc_boots[,1]), col = pal["GG"], lwd = 2, main = "IBS BS Distribution", xlab = "IBS")
lines(density(ibsc_boots[,2]), col = pal["GG2"], lwd = 2)
lines(density(ibsc_boots[,3]), col = pal["CPH"], lwd = 2)
lines(density(ibsc_boots[,4]), col = pal["RSF"], lwd = 2)
lines(density(ibsc_boots[,4]), col = pal["KM0"], lwd = 2)
abline(v = calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_gg, ibs_times, 34*365.25/12, 7*365.25/12)$ibs, col = pal["GG"], lwd = 1)
abline(v = calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_gg2, ibs_times, 34*365.25/12, 7*365.25/12)$ibs, col = pal["GG2"], lwd = 1)
abline(v = calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_cph, ibs_times, 34*365.25/12, 7*365.25/12)$ibs, col = pal["CPH"], lwd = 1)
abline(v = calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_rsf, ibs_times, 34*365.25/12, 7*365.25/12)$ibs, col = pal["RSF"], lwd = 1)
abline(v = calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_km0, ibs_times, 34*365.25/12, 7*365.25/12)$ibs, col = pal["KM0"], lwd = 1)
legend("topright", legend = c("GG", "GG2", "CPH", "RSF", "KM0"), col = pal[c("GG", "GG2", "CPH", "RSF", "KM0")], lty = "solid", inset = 0.05)

plot(density(ibsc_boots[,5] - ibsc_boots[,1]), col = pal["GG"], lwd = 2, main = "IBS\\_KM0 - IBS\\_x BS Distribution", xlab = "IBS", ylim = c(0, 0.1))
lines(density(ibsc_boots[,5] - ibsc_boots[,2]), col = pal["GG2"], lwd = 2)
lines(density(ibsc_boots[,5] - ibsc_boots[,3]), col = pal["CPH"], lwd = 2)
lines(density(ibsc_boots[,5] - ibsc_boots[,4]), col = pal["RSF"], lwd = 2)
abline(v = (calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_km0, ibs_times, max(data.val$Time))$ibs - calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_gg, ibs_times, max(data.val$Time))$ibs), col = pal["GG"], lwd = 1)
abline(v = (calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_km0, ibs_times, max(data.val$Time))$ibs - calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_gg2, ibs_times, max(data.val$Time))$ibs), col = pal["GG2"], lwd = 1)
abline(v = (calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_km0, ibs_times, max(data.val$Time))$ibs - calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_cph, ibs_times, max(data.val$Time))$ibs), col = pal["CPH"], lwd = 1)
abline(v = (calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_km0, ibs_times, max(data.val$Time))$ibs - calcIBS(Surv(data.val$Time, data.val$DSD), ibs_preds_rsf, ibs_times, max(data.val$Time))$ibs), col = pal["RSF"], lwd = 1)
legend("topright", legend = c("GG", "GG2", "CPH", "RSF"), col = pal[c("GG", "GG2", "CPH", "RSF")], lty = "solid", inset = 0.05)
@
...
ibsc_boots2 = boot(data.val, statistic = function(d, i) {
	...
	gg2 = calcIBS(Surv(d$Time, d$DSD)[i,], ibs_preds_gg2[i,], ibs_times, 34*365.25/12, 7*365.25/12)$ibs
...
head(data.all)
#fit.final.gg = flexsurvreg(Surv(Time, DSD) ~ SexM + AgeCent + LocBody + SizeCent + A2 + A4 + SizeCent:AgeCent + SexM:SizeCent,
fit.final.gg = flexsurvreg(Surv(Time, DSD) ~ SexM + LocBody + SizeCent + A2 + A4,
	anc = list(
		sigma = ~ SexM,
		Q = ~ SexM),
	data = data.all, dist = "gengamma")
# fit.final.cph = coxph(Surv(Time, DSD) ~ strata(SexM)+AgeCent+LocBody+SizeCent+A2+A4+SizeCent:AgeCent+strata(SexM):SizeCent, data = data.all, x = TRUE, y = TRUE, model = TRUE)
fit.final.cph = coxph(Surv(Time, DSD) ~ strata(SexM) + LocBody + SizeCent + A2 + A4, data = data.all, x = TRUE, y = TRUE, model = TRUE)
set.seed(20150208)
...
saveRDS(list(gg = fit.final.gg, km0 = fit.final.km0, cph = fit.final.cph, rsf = fit.final.rsf, data.train = data.all), file = "05_final_model.rds")
@
