...
<<eda-ph-check-full>>=
fit.cph = coxph(Surv(Time, DSD) ~ SexM + AgeCent + AgeCent2 + LocBody + SizeCent + SizeSmall + A2 + A4, data = data)
cox.zph(fit.cph)
plot(cox.zph(fit.cph))
@

<<eda-ph-check-full-sexplot>>=
...
<<eda-ph-check-full-2>>=
anova(coxph(Surv(Time, DSD) ~ SexM*(AgeCent + AgeCent2 + LocBody + SizeCent + SizeSmall + A2 + A4), data = data))
@
Nope, good.  We're not interested in gender effects so just stratify.

<<eda-ph-check-full-3>>=
fit.cph = coxph(Surv(Time, DSD) ~ strata(SexM) + AgeCent + AgeCent2 + LocBody + SizeCent + SizeSmall + A2 + A4, data = data)
cox.zph(fit.cph)
plot(cox.zph(fit.cph))
@
...
#fit.cph.as = glmulti(Surv(Time, DSD) ~ strata(SexM) + AgeCent + AgeCent2 + LocBody + SizeCent + SizeSmall + A2 + A4, data = data, marginality = TRUE, method = "h", fitfunction = "coxph", crit = "bic", level = 2)
set.seed(20150110)
fit.cph.as = glmulti(Surv(Time, DSD) ~ strata(SexM) + AgeCent + AgeCent2 + LocBody + SizeCent + SizeSmall + A2 + A4, data = data, marginality = TRUE, method = "g", fitfunction = "coxph", crit = "bic", level = 2, plotty = FALSE, report = FALSE)
# fit.cph.as
...
temp.linpred.gg = summary(fit.gg, newdata = data.val, type = "survival")[,1]
temp.linpred.gg2 = summary(fit.gg2, newdata = data.val, type = "survival")[,1]
temp.linpred.cph = predict(fit.cph, newdata = data.val)

summary(coxph(Surv(Time, DSD) ~ temp.linpred.gg, data.val))
summary(coxph(Surv(Time, DSD) ~ temp.linpred.gg2, data.val))
summary(coxph(Surv(Time, DSD) ~ temp.linpred.cph, data.val))

anova(coxph(Surv(Time, DSD) ~ offset(temp.linpred.gg) + temp.linpred.gg, data.val))
anova(coxph(Surv(Time, DSD) ~ offset(temp.linpred.gg2) + temp.linpred.gg2, data.val))
anova(coxph(Surv(Time, DSD) ~ offset(temp.linpred.cph) + temp.linpred.cph, data.val))

summary(coxph(Surv(Time, DSD) ~ offset(temp.linpred.gg) + SexM + AgeCent + LocBody + SizeCent + A2 + A4, data.val))
summary(coxph(Surv(Time, DSD) ~ offset(temp.linpred.gg2) + SexM + AgeCent + LocBody + SizeCent + A2 + A4, data.val))
summary(coxph(Surv(Time, DSD) ~ offset(temp.linpred.cph) + SexM + AgeCent + LocBody + SizeCent + A2 + A4, data.val))
@


<<model-selection-auc>>=
temp.gg = timeROC(data.val$Time/365.25*12, data.val$DSD, temp.linpred.gg, cause = 1, times = seq(1, 36, 1), iid = TRUE)
temp.gg2 = timeROC(data.val$Time/365.25*12, data.val$DSD, temp.linpred.gg2, cause = 1, times = seq(1, 36, 1), iid = TRUE)
temp.cph = timeROC(data.val$Time/365.25*12, data.val$DSD, temp.linpred.cph, cause = 1, times = seq(1, 36, 1), iid = TRUE)
