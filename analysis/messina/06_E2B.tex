...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Loading required package: foreach\\\#\# Loading required package: iterators\\\#\# Loading required package: parallel}}\end{kframe}
\end{knitrout}
...
\hlstd{e2b.design} \hlkwb{=} \hlkwd{expand.grid}\hlstd{(}
        ...
        \hlkwc{R1} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,} \hlnum{2}\hlstd{,} \hlnum{4}\hlstd{),}
        ...
        \hlkwc{reps} \hlstd{=} \hlnum{1e1}\hlstd{)}
...
\hlstd{e2b.det} \hlkwb{=} \hlkwd{mlply}\hlstd{(e2b.design,} \hlkwa{function}\hlstd{(}\hlkwc{Delta}\hlstd{,} \hlkwc{R1}\hlstd{,} \hlkwc{sigma_epsilon}\hlstd{,} \hlkwc{pc}\hlstd{,} \hlkwc{p1}\hlstd{,} \hlkwc{stat.alpha}\hlstd{,} \hlkwc{messina.minmarg}\hlstd{,} \hlkwc{messina.objective}\hlstd{,} \hlkwc{n}\hlstd{,} \hlkwc{reps}\hlstd{,} \hlkwc{jobindex}\hlstd{,} \hlkwc{...}\hlstd{) \{}
        \hlcom{#message(date(), "\textbackslash{}t", jobindex)}
        \hlkwd{rowMeans}\hlstd{(}\hlkwd{replicate}\hlstd{(reps, \{ data} \hlkwb{=} \hlkwd{e2b.datafun}\hlstd{(n, p1, pc, Delta, R1, sigma_epsilon);} \hlkwd{e2b.detfun}\hlstd{(data}\hlopt{$}\hlstd{x, data}\hlopt{$}\hlstd{y, stat.alpha, messina.objective, messina.minmarg) \})) \},} \hlkwc{.parallel} \hlstd{=} \hlnum{TRUE}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in do.ply(i): task 100 failed - "{}missing value where TRUE/FALSE needed"{}}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in lapply(x, length): object 'e2b.det' not found}}\end{kframe}
\end{knitrout}
