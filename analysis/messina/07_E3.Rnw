...
library(plyr)
APGI.messina = messinaSurv(APGI.x, APGI.y, messinaSurvObj.CoxCoef(round(log(2), 3)), parallel = TRUE, silent = FALSE, seed = 20150321)
...
doValidation = function(train.features, train.x, train.threshold, train.merit, min_merit, test.features, test.x, test.y)
{
	...
	val.test.y = test.y[val.perm,,drop=FALSE]
	...
	val.chisq = mapply(function(row_index, threshold) {
		...
		fit = survdiff(y ~ I(x > threshold))
		fit$chisq
	}, 1:length(val.test.threshold), val.test.threshold)

	data.frame(merit = val.train.merit, threshold.train = val.train.threshold, threshold.test = val.test.threshold, chisq = val.chisq)
}

debug(doValidation)
val.GSE28735.messina = doValidation(APGI.feats$symbol, APGI.messina@fits@summary$threshold, APGI.messina@fits@summary$margin, 1, GSE28735.feats$Gene.symbol, GSE28735.xreg, GSE28735.y)
val.GSE28735.maxstat = doValidation(APGI.feats$symbol, sapply(APGI.maxstat, function(x) x$threshold), -log10(sapply(APGI.maxstat, function(x) x$p.value)), -log10(0.05), GSE28735.feats$Gene.symbol, GSE28735.xreg, GSE28735.y)
val.GSE21501.messina = doValidation(APGI.feats$symbol, APGI.messina@fits@summary$threshold, APGI.messina@fits@summary$margin, 1, GSE21501.feats$Gene.symbol, GSE21501.xreg, GSE21501.y)
val.GSE21501.maxstat = doValidation(APGI.feats$symbol, sapply(APGI.maxstat, function(x) x$threshold), -log10(sapply(APGI.maxstat, function(x) x$p.value)), -log10(0.05), GSE21501.feats$Gene.symbol, GSE21501.xreg, GSE21501.y)
