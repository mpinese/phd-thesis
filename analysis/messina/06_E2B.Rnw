...
e2b.detfun = function(x, y, stat.alpha, messina.objective, messina.minmarg, ...)
{
	...
	fit.messina = messinaSurv(x.messina, y, messina_objectives[[messina.objective]], silent = TRUE)
...
set.seed(20150320)
e2b.det = mlply(e2b.design, function(Delta, R1, sigma_epsilon, pc, p1, stat.alpha, messina.minmarg, messina.objective, n, reps, ...) { 
	rowMeans(replicate(reps, { data = e2b.datafun(n, p1, pc, Delta, R1, sigma_epsilon); e2b.detfun(data$x, data$y, stat.alpha, messina.objective, messina.minmarg) })) })
...
ggplot(e2b.design, aes(x = margin, y = detrate, colour = factor(method))) + geom_line(lwd = 1) + xlab("Margin") + ylab("Detection rate") + labs(colour = "Method") + theme_bw() + facet_wrap(~ n) + geom_vline(xintercept = 1, alpha = 0.5, linetype = "dashed")
@
