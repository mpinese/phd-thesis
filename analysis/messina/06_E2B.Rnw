...
e2b.detfun = function(x, y, stat.alpha, messina.objective, messina.minmarg, ...)
{
	...
	c(maxstat = det.maxstat, c1 = det.1cut, c10 = det.10cut, cH10 = det.10cutHolm, m = det.messina)
}
...
e2b.repfun = function(i, Delta, R1, sigma_epsilon, pc, p1, stat.alpha, messina.minmarg, messina.objective, n, ...)
{
	...
	if(class(dets) == "try-error")	{ return(c(NA, NA, NA, NA, NA)) }
	return(dets)
}
...
set.seed(20150320)
e2b.det = mlply(e2b.design, e2b.expfun, .parallel = TRUE, .inform = TRUE)
...
ggplot(e2b.design[e2b.design$n == 25,], aes(x = margin, y = detrate, colour = factor(method))) + geom_line(lwd = 1) + xlab("Margin") + ylab("Detection rate") + labs(colour = "Method") + theme_bw() + geom_vline(xintercept = 1, alpha = 0.5, linetype = "dashed") + facet_grid(R1 ~ p1 ~ pc)
ggplot(e2b.design[e2b.design$n == 50,], aes(x = margin, y = detrate, colour = factor(method))) + geom_line(lwd = 1) + xlab("Margin") + ylab("Detection rate") + labs(colour = "Method") + theme_bw() + geom_vline(xintercept = 1, alpha = 0.5, linetype = "dashed") + facet_grid(R1 ~ p1 ~ pc)
ggplot(e2b.design[e2b.design$n == 100,], aes(x = margin, y = detrate, colour = factor(method))) + geom_line(lwd = 1) + xlab("Margin") + ylab("Detection rate") + labs(colour = "Method") + theme_bw() + geom_vline(xintercept = 1, alpha = 0.5, linetype = "dashed") + facet_grid(R1 ~ p1 ~ pc)
ggplot(e2b.design[e2b.design$n == 200,], aes(x = margin, y = detrate, colour = factor(method))) + geom_line(lwd = 1) + xlab("Margin") + ylab("Detection rate") + labs(colour = "Method") + theme_bw() + geom_vline(xintercept = 1, alpha = 0.5, linetype = "dashed") + facet_grid(R1 ~ p1 ~ pc)
@
