...
\hlstd{PARSE_exact_scores} \hlkwb{=} \hlnum{1.354} \hlopt{*} \hlstd{(coefs.diag_dsd[}\hlnum{1}\hlstd{, ]} \hlopt{-} \hlstd{coefs.diag_dsd[}\hlnum{5}\hlstd{, ])} \hlopt{+} \hlnum{1.548} \hlopt{*}
    \hlstd{(coefs.diag_dsd[}\hlnum{6}\hlstd{, ]} \hlopt{-} \hlstd{coefs.diag_dsd[}\hlnum{2}\hlstd{, ])}
...
\begin{kframe}\begin{alltt}
\hlkwd{plot}\hlstd{(temp.pred.pairs[,} \hlstr{"mg.1"}\hlstd{]} \hlopt{~} \hlstd{temp.pred.pairs[,} \hlstr{"PCNA"}\hlstd{],} \hlkwc{col} \hlstd{=} \hlkwd{ifelse}\hlstd{(}\hlkwd{rownames}\hlstd{(temp.pred.pairs)} \hlopt{%in%}
...
\hlstd{doValForSingleCancer} \hlkwb{=} \hlkwa{function}\hlstd{(}\hlkwc{cancer_id}\hlstd{) \{}
    ...
    \hlkwa{if} \hlstd{(}\hlopt{!}\hlstr{"illuminahiseq_rnaseqv2"} \hlopt{%in%} \hlkwd{names}\hlstd{(cancer_data}\hlopt{$}\hlstd{gex)) \{}
...
\hlstd{sig.emt_groger_neg} \hlkwb{=} \hlkwd{c}\hlstd{(}\hlstr{"CD24"}\hlstd{,} \hlstr{"CXCL16"}\hlstd{,} \hlstr{"DSG3"}\hlstd{,} \hlstr{"ELF3"}\hlstd{,} \hlstr{"EPCAM"}\hlstd{,} \hlstr{"EPHA1"}\hlstd{,} \hlstr{"JUP"}\hlstd{,}
    ...
    \hlstr{"SERPINB1"}\hlstd{,} \hlstr{"SLPI"}\hlstd{,} \hlstr{"TMEM30B"}\hlstd{,} \hlstr{"TPD52L1"}\hlstd{,} \hlstr{"IFI30"}\hlstd{,} \hlstr{"IL18"}\hlstd{,} \hlstr{"KLK7"}\hlstd{,} \hlstr{"KRT15"}\hlstd{,}
    \hlstr{"KRT17"}\hlstd{,} \hlstr{"LSR"}\hlstd{,} \hlstr{"MAP7"}\hlstd{,} \hlstr{"MBP"}\hlstd{,} \hlstr{"OCLN"}\hlstd{,} \hlstr{"PKP2"}\hlstd{,} \hlstr{"PPL"}\hlstd{,} \hlstr{"PRSS8"}\hlstd{,} \hlstr{"RAPGEF5"}\hlstd{,}
    \hlstr{"SPINT1"}\hlstd{,} \hlstr{"PRRG4"}\hlstd{,} \hlstr{"GPX3"}\hlstd{,} \hlstr{"SLC27A2"}\hlstd{,} \hlstr{"SMPDL3B"}\hlstd{,} \hlstr{"SORL1"}\hlstd{,} \hlstr{"ST6GALNAC2"}\hlstd{,}
    \hlstr{"RHOD"}\hlstd{,} \hlstr{"AGR2"}\hlstd{,} \hlstr{"C10orf10"}\hlstd{,} \hlstr{"CDS1"}\hlstd{,} \hlstr{"FAM169A"}\hlstd{,} \hlstr{"FXYD3"}\hlstd{,} \hlstr{"KLK10"}\hlstd{,} \hlstr{"LAD1"}\hlstd{,}
    \hlstr{"MTUS1"}\hlstd{,} \hlstr{"PLS1"}\hlstd{)}
\hlstd{x.medcent} \hlkwb{=} \hlstd{x} \hlopt{-} \hlkwd{apply}\hlstd{(x,} \hlnum{1}\hlstd{, median)}
\hlstd{emt.scores.pos} \hlkwb{=} \hlkwd{apply}\hlstd{(x.medcent[}\hlkwd{rownames}\hlstd{(x.medcent)} \hlopt{%in%} \hlstd{sig.emt_groger_pos,}
    \hlstd{],} \hlnum{2}\hlstd{, median)}
\hlstd{emt.scores.neg} \hlkwb{=} \hlkwd{apply}\hlstd{(x.medcent[}\hlkwd{rownames}\hlstd{(x.medcent)} \hlopt{%in%} \hlstd{sig.emt_groger_neg,}
    \hlstd{],} \hlnum{2}\hlstd{, median)}
