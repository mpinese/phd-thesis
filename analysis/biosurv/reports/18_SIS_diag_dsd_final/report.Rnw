...
<<val-tests>>=
temp = coxph(Surv(GSE21501.samp$time, GSE21501.samp$event) ~ predict(glmnet.fit.cv.diag_dsd$glmnet.fit, newx = t(GSE21501.coefs), s = glmnet.fit.cv.recr_dsd$lambda.1se))
summary(temp)
temp = coxph(Surv(GSE28735.samp$time, GSE28735.samp$event) ~ predict(glmnet.fit.cv.diag_dsd$glmnet.fit, newx = t(GSE28735.coefs), s = glmnet.fit.cv.recr_dsd$lambda.1se))
summary(temp)
@
...
cpv.pvals = apply(coef(nmf.final), 1, function(mg) sapply(cpvs.diag_dsd, function(x) {
        s = !is.na(mg) & !is.na(x)
        x = x[s]
        mg = mg[s]
...
apply(coef(nmf.final), 1, function(c1) cor.test(c1, samps$purity_qpure, method = "kendall")$p.value)
p.adjust(apply(coef(nmf.final), 1, function(c1) cor.test(c1, samps$purity_qpure, method = "kendall")$p.value), "BH")
apply(coef(nmf.final), 1, function(c1) cor(c1, samps$purity_qpure, method = "kendall", na.rm = TRUE))

#termplot(lm(coef(nmf.final)[6,] ~ cpvs.diag_dsd$Path.Grade.Coarse), se = TRUE, rug = TRUE)
