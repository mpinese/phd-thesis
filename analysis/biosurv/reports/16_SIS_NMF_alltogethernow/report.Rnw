...
{
	summary(coxph(y.diag_rec ~ coefs.diag_rec[,i]))
}
@
...
{
	summary(coxph(y.diag_dsd ~ coefs.diag_dsd[,i]))
}
@
...
{
	summary(coxph(y.recr_dsd ~ coefs.recr_dsd[,i]))
}
@
...
colnames(temp.pvals.FWER) = gsub("$", ".Holm", colnames(temp.pvals.FWER))
temp.pvals.BY = matrix(p.adjust(as.vector(temp.pvals), "BY"), nrow = nrow(temp.pvals))
colnames(temp.pvals.BY) = gsub("$", ".BY", colnames(temp.pvals.FWER))
xlin.scaled.sel.nmf.cpv.pvals = rbind(xlin.scaled.sel.nmf.cpv.pvals, temp.pvals.FWER, temp.pvals.BY)
...
\subsubsection{Outcome: Diagnosis to recurrence}
\subsubsubsection{All-subsets regression}
...
\subsubsubsection{Adaptive LASSO}
