...
load("05_gexnormed_cpv_matched.rda")
...
library(VennDiagram)
venn.diagram(list(
	AU = which(sel.patient.au), 
	PDAC = which(sel.patient.pdac), 
	SurgOK = which(sel.patient.surgok), 
	#FullFU = which((!is.na(cpvs$Surv.EventTimeFromDiag.Death)) & (!is.na(cpvs$Surv.Event.DSDeath)))
	), filename = "07_diag_dsd.tif")
...
calcSingleCategoryGSVAScores = function(category_id, x, rnaseq)
{
	...
	message("  Loading GMT")
	gmt = getGmt(sprintf("MSigDB/%s.all.v4.0.symbols.gmt", category_id), 
...
save.image("07_data_for_SIS.rda")
