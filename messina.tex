...
\begin{algorithm}
  \KwData{An $n$-tuple of covariate measurements $x$, an $n$-tuple of associated dependent values $y$, a $m$-vector of candidate cutpoints $c$, and an objective function $f: (\mathbb{B}^n, \mathbb{Y}^n) \rightarrow \mathbb{B}$.  $x$ and $c$ are to be in ascending order.  The range of $y$ is given as $\mathbb{Y}^n$, as it varies between modes of Messina.}
...
\begin{algorithm}
  ...
  \KwResult{$(t^* \in \mathbf{R}, \Delta^* \in [0, \infty))$}
