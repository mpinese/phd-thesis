...
\begin{algorithm}
  \KwData{An $m$-vector of normal locus read depths $n$, an $m$-vector of tumour locus read depths $t$, minimum normal sample depth $mindepth$.}
  \KwResult{An $m$-vector of floats: for each locus, the one-sided P-value for \acrshort{CNV} loss at that locus, $p_{loss}$.}

  \Begin {
    \tcp{Estimate $k$ so that the modal ploidy signal will be called as diploid}
    $S \longleftarrow \{ t_i/n_i : i \in \{1 \dots m\} \wedge n_i \geq mindepth \}$\;
    $\hat{k} \longleftarrow mode(S)$\;
    
    \tcp{Calculate P-values.  $W_5$ and $\Phi$ are as defined in the text.}
    $P \longleftarrow \text{m-vector of NAs}$\;
