...
\begin{align}
  T_i &\approxdist f Pois\left(d_T b_i c_i k \right) + (1-f) Pois\left(d_T b_i\right) \\
      &\equiv Pois\left(d_T b_i \left( f \left( c_i k - 1 \right) + 1 \right) \right) \label{eq:comp_cnv_Tpois}
\end{align}
...
\mpfatal{This maths needs to be redone.  In particular, $R_{H0}$ is probably going to be different for loss vs gain.  Need less hangover}
\begin{equation}
