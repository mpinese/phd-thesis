...
\mpnote{Say something about K here?  Main points: K encodes the expected signal of diploid DNA, and is present as a consequence of the interaction of variable ploidy with the input normalization to total DNA quantity instead of total cell count.  Due to the underdetermined nature of the system when the number of input cells is unknown, K must be estimated from the data.  There is no way to conclusively determine the correct K, due to the general indeterminancy of the CNV genotyping problem.  Therefore, we use the heurestic that the most common copy number will be diploid, when determining a value for K.}
