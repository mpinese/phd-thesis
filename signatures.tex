...
500 random restarts of rank 6 \gls{SNMFL} were performed on the survival-associated gene matrix to yield the final factorization.  The resultant clustering consensus matrix was highly stable (figure \ref{fig:nmf_consensus}), and the basis matrix $W$ was reasonably sparse (figure \ref{fig:nmf_basis}).  Low row L1 norm of the basis matrix is a desirable condition for this analysis, as it indicates that metagenes are largely distinct transcriptional modules, with little overlap in terms of shared transcripts with high loadings.  The desire for a small row L1 norm on $W$ was the motivating factor for the choice of \gls{SNMFL} for \gls{NMF} of these data.
...
\label{fig:nmf_basis}
\caption{Basis matrix of the final \acrshort{SNMFL} factorization.  Rows represent genes, and columns metagenes, with cell colours proportional to the loading of a given gene on a given metagene.  The loadings are sparse within rows, indicating that the metagenes are modular, each affecting the expression of largely distinct sets of target genes.}
\end{figure}

\paragraph{Identifying prognostic metagenes}
The transcription patterns of genes associated with survival in the \gls{APGI} cohort could be decomposed into just six largely distinct metagenes.  I then set out to determine the influence of each of these metagenes individually on outcome and other clinical variables.  Coefficients for each of the six metagenes on each sample were recalculated by \gls{NNLS}, to ensure that the same methods were used in both training and validation arms of the study.  

Multivariate Cox regression was performed to determine the influence of the six metagenes on three clinical intervals (time from diagnosis to recurrence, recurrence to disease-specific death, and diagnosis to disease-specific-death)
