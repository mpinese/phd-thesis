...
Proliferation and the \gls{EMT} are two of the ten major hallmarks of cancer \cite{Hanahan2011}, and so it is unsurprising that they are so closely associated with patient survival: the very definition of cancer involves uncontrolled and unceasing cell proliferation (TODO cite); and the \gls{EMT} is a major enabling step in metastasis, the process by which most cancers are ultimately lethal (TODO cite).  What \emph{is} surprising is that the majority of hallmarks do \emph{not} appear to be strongly associated with survival in resected \gls{PDAC}.  In particular, the absence of stromal or inflammatory signatures is unexpected given that \gls{PDAC} cells are almost always surrounded by extensive stroma, which is believed to be a clinically significant component of the disease \cite{Luo2012}.

Cancer is fundamentally a proliferative process: it is through inappropriate and continued proliferation, and the consequent destruction of normal tissues and disruption of homeostasis, that cancer progressively overwhelms the body.  In keeping with this, the meta-PCNA signature of cell proliferation is prognostic in a wide range of cancers \cite{Venet2011} -- actually it isn't tested as such in Venet.
TODO: should probably do what Venet suggests and pull some random genes as a test.
* Something on prolif.  Obviously a central aspect of cancer.  Discuss meta-PCNA briefly and its pan-can applicability.  A1/A2 also have pan-can prognostic ability, but interestingly it's subtly different from PCNA -- eg kirc, where double pos is needed for poor prognosis.
